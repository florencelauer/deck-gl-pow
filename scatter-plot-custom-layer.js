...
varying float stroke_opacity;
varying vec3 v_data;
varying float v_visibility;
`,

      // compute the progress for each vertex
      'vs:#main-end': `\
vec2 extrude = vec2(geometry.uv);
lowp float antialiasblur = 1.0 / project_uDevicePixelRatio / (radius + stroke_width);
v_data = vec3(extrude.x, extrude.y, antialiasblur);
stroke_color = a_stroke_color;
stroke_opacity = a_stroke_opacity;
color = a_color;
`,
...
`,
      ...
      int draw_border = 0;
      float f_stroke_opacity = stroke_opacity;
      ...
      for (int i = 0; i < 3; i++)
      {
        ...
        if ((rotating_angle_max > TPI && angle >= 0.0 && angle < rotating_angle_max - TPI) || (angle >= rotating_angle_min && angle < rotating_angle_max))
        {
          ...
          {
            f_stroke_opacity = stroke_opacity * (angle + TPI - rotating_angle_min) / (arc);
          }
          else
          {
            f_stroke_opacity = stroke_opacity * (angle - rotating_angle_min) / (arc);
          }
          draw_border = 1;
        }
      }
      ...
      else if (extrude_length <= third_step)
      {
        ...
        gl_FragColor = opacity_t * stroke_color * f_stroke_opacity * 0.5;
      }
      else
      {
...
`,
  ...
  draw(params) {
    params.uniforms.u_time = (performance.now() / 1000) % 100000;
    super.draw(params);
  }
}
